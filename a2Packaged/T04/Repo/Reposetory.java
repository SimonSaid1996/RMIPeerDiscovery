...
public class Reposetory implements IDistributedRepository {
    ...
    public void setMycon(Connector c){
        myCon = c;
    }
...
//                    return("you dont need to put repid of this rep as well");
            ...
            if ((keyParts.length > 1) && !(keyParts[0].equals(id))) {//comand is for other repo
                ...
                System.out.println("want to command this to other repo " + String.join(" ", command) + "/");
                String res = otherrepo.connection(String.join(" ", command));
                System.out.println("comand is finish result : " + res);
                ...
                try{
                    ...
                    case "ENUMKEYS":
                        if(command.length==1){
                            this.enumKeys(myCon);
                            return ("OK "+this.list_keys().toString() );
                        }
                        else{
                            return(new RepException(new IllegalArgumentException()).getErrormsg());
                        }
                    case "ENUMVALUES":
                        if(command.length==2){
                            ...
                            return ("OK "+ this.getValues(command[1]).toString());
                        }
                        else{
                            return(new RepException(new IllegalArgumentException()).getErrormsg());
                        }
...
//        System.out.println("bka nla bla");
